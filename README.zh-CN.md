...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `mdWorkbench.themeMode` | string | `auto` | 主题模式：`auto` / `light` / `dark` |
| `mdWorkbench.previewStyle` | string | `default` | 预览风格：`default` / `github` / `notion` / `tokyo-night` / `obsidian` |
| `mdWorkbench.showToc` | boolean | `true` | 是否显示目录侧边栏 |
